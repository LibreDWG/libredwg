...
api_process (dwg_object *obj)
{
  ...
  num_owned = dwg_ent_polyline_mesh_get_num_owned (polyline_mesh, &error);
  if (!error)
  ...
  // returns curve type
  curve_type = dwg_ent_polyline_mesh_get_curve_type (polyline_mesh, &error);
  if (!error)
    printf ("curve type of polyline_mesh : %ud\n", curve_type);
  else
    printf ("in reading curve type \n");

  m_density = dwg_ent_polyline_mesh_get_m_density (polyline_mesh, &error);
  if (!error)
    printf ("M density of polyline_mesh : %ud\n", m_density);
  else
    printf ("in reading m_density \n");

  n_density = dwg_ent_polyline_mesh_get_n_density (polyline_mesh, &error);
  if (!error)
    printf ("n density of polyline_mesh : %ud\n", n_density);
  else
    printf ("in reading n density \n");

  num_m_verts = dwg_ent_polyline_mesh_get_num_m_verts (polyline_mesh, &error);
  if (!error)
    printf ("num m verts of polyline_mesh : %ud\n", num_m_verts);
  else
    printf ("in reading m vert count \n");

  num_n_verts = dwg_ent_polyline_mesh_get_num_n_verts (polyline_mesh, &error);
  if (!error)
