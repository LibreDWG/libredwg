...
low_level_process (dwg_object *obj)
{
  ...
  printf ("extrusion of mtext : x = %f, y = %f, z = %f\n", mtext->extrusion.x,
          mtext->extrusion.y, mtext->extrusion.z);
  printf ("insertion point of mtext : x = %f, y = %f, z = %f\n",
          mtext->insertion_pt.x, mtext->insertion_pt.y, mtext->insertion_pt.z);
  printf ("x_axis_dir of mtext : x = %f,y = %f,z = %f\n", mtext->x_axis_dir.x,
          mtext->x_axis_dir.y, mtext->x_axis_dir.z);
}
...
api_process (dwg_object *obj)
{
  ...
  linespace_style = dwg_ent_mtext_get_linespace_style (mtext, &error);
  if (!error)
    {
      printf ("linespace style of mtext : %ud\n", linespace_style);
    }
  ...
  linespace_factor = dwg_ent_mtext_get_linespace_factor (mtext, &error);
  if (!error)
    printf ("linespace factor of mtext : %f\n", linespace_factor);
  else
