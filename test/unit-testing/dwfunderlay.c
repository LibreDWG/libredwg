...
api_process (dwg_object *obj)
{
  ...
  if (!dwg_dynapi_entity_value (underlay, "DWFUNDERLAY", "clip_verts", &clip_verts, NULL))
    fail ("DWFUNDERLAY.clip_verts");
  else
    {
      for (BITCODE_BL i = 0; i < num_clip_verts; i++)
        {
          ok ("DWFUNDERLAY.clip_verts[%d]: (%f, %f)", i, clip_verts[i].x, clip_verts[i].y);
        }
    }
  CHK_ENTITY_TYPE (underlay, DWFUNDERLAY, num_clip_inverts, BS);
  if (!dwg_dynapi_entity_value (underlay, "DWFUNDERLAY", "clip_inverts", &clip_inverts, NULL))
