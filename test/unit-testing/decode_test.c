...
read_compressed_bytes_tests (void)
{
  ...
  for (int i=0; i < ARRAY_SIZE(tests); i++) {
    Bit_Chain bitchain = strtobt (tests[i].bits);
    int result = read_compressed_bytes (&bitchain, tests[i].opcode, tests[i].mask);
    if (result == tests[i].result)
      {
        if (i == ARRAY_SIZE (tests) - 1)
...
decompress_R2004_section_tests (void)
{
  ...
  unsigned char decomp_auxh_bin[123] = {
    0xff, 0x88, 0x01, 0x21, 0x00, 0x1d, 0x00, 0x19, 0x00, 0x00, 0x00, 0xff,
    0xff, 0xff, 0xff, 0x0f, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x16,
    0x00, 0x2e, 0x00, 0x16, 0x00, 0x2e, 0x00, 0x04, 0x00, 0x65, 0x05, 0x00,
    0x00, 0x00, 0x00, 0x02, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x02,
    0x00, 0x07, 0x00, 0xea, 0x74, 0x25, 0x00, 0x9a, 0xe6, 0x33, 0x04, 0xb0,
    0x82, 0x25, 0x00, 0xe0, 0x1c, 0xf7, 0x01, 0xe9, 0x0b, 0x00, 0x00, 0x00,
  ...
  unsigned char comp_ofs_bin[181] = {
    0x00, 0x04, 0x00, 0x00, 0xff, 0xff, 0x00, 0x00, 0xff, 0xff, 0xb0, 0x82,
    0x25, 0x00, 0xe0, 0x1c, 0xf7, 0x01, 0x00, 0x00, 0x00, 0x00, 0x04, 0x32,
    0x54, 0x01, 0x01, 0x00, 0x00, 0x00, 0x64, 0x8e, 0x01, 0x00, 0x02, 0x70,
    0x02, 0x01, 0xff, 0xff, 0xff, 0xff, 0x74, 0x02, 0x20, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x0f, 0x14, 0x00, 0x0a, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x11, 0x00,
    0x00
  };
  unsigned char decomp_ofs_bin[53] = {
    0x00, 0x00, 0xff, 0xff, 0x00, 0x00, 0xff, 0xff, 0xb0, 0x82, 0x25, 0x00,
    0xe0, 0x1c, 0xf7, 0x01, 0x00, 0x00, 0x00, 0x00, 0x04, 0x32, 0x00, 0x00,
  ...
  if (result == 0 && dec.size == sizeof decomp_auxh_bin && memcmp (dec.chain, decomp_auxh_bin, sizeof decomp_auxh_bin) == 0)
    ...
    fail ("decompress_R2004_section auxh %d %lu", result, (unsigned long)dec.size);
  ...
  dec.byte = 0;
  result = decompress_R2004_section (&src, &dec);
  if (result == 0 && memcmp (dec.chain, decomp_ofs_bin, sizeof decomp_ofs_bin) == 0)
    ok ("decompress_R2004_section");
  else
    fail ("decompress_R2004_section ofs %d %lu", result, (unsigned long)dec.size);
}
