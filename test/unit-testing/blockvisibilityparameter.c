...
api_process (dwg_object *obj)
{
  ...
  if (!dwg_dynapi_entity_value (_obj, "BLOCKVISIBILITYPARAMETER", "states",
    ...
    for (i = 0; i < num_states; i++)
      {
        CHK_SUBCLASS_UTF8TEXT (states[i], BLOCKVISIBILITYPARAMETER_state, name);
        CHK_SUBCLASS_TYPE (states[i], BLOCKVISIBILITYPARAMETER_state, num_blocks, BL);
        CHK_SUBCLASS_HV (states[i], BLOCKVISIBILITYPARAMETER_state, blocks, states[i].num_blocks);
