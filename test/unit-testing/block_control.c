...
api_process (dwg_object *obj)
{
  ...
  dwg_obj_block_control *ctrl = dwg_object_to_BLOCK_CONTROL (obj);

  CHK_ENTITY_TYPE (ctrl, BLOCK_CONTROL, num_entries, BS, num_entries);
  entries = dwg_object_tablectrl_get_entries (obj, &error);
  if (!dwg_dynapi_entity_value (ctrl, "BLOCK_CONTROL", "entries", &hdls, NULL))
  ...
  CHK_ENTITY_TYPE (ctrl, BLOCK_CONTROL, objid, BL, objid);  
  CHK_ENTITY_H (ctrl, BLOCK_CONTROL, model_space, model_space);
  CHK_ENTITY_H (ctrl, BLOCK_CONTROL, model_space, model_space);
}
