...
api_process (dwg_object * obj)
{
  ...
  dwg_ent_dim *dim = (dwg_ent_dim *)dim_diameter;
  ...
  ecs11 =
    dwg_ent_dim_get_elevation_ecs11 (dim, &error);
  if (!error  && dim->elevation.ecs_11 == ecs11)
    pass ("Working Properly");
  else
    fail ("error in reading ecs11");

  ecs12 = dwg_ent_dim_get_elevation_ecs12 (dim, &error);
  if (!error  && dim->elevation.ecs_12 == ecs12)	// error check
    pass ("Working Properly");
  else
  ...
  dwg_ent_dim_get_ins_scale (dim, &ins_scale,
				      &error);
  ...
  if (!error  && text_mid_pt.x == dim->text_midpt.x && text_mid_pt.y == dim->text_midpt.y)	// error checking
    pass ("Working Properly");
  else
  ...
  user_text =
    dwg_ent_dim_get_user_text (dim, &error);
  ...
  text_rot =
    dwg_ent_dim_get_text_rot (dim, &error);
  ...
  flip_arrow1 = dwg_ent_dim_get_flip_arrow1 (dim,
						      &error);
  ...
  flip_arrow2 = dwg_ent_dim_get_flip_arrow2 (dim,
						      &error);
  if (!error  && dim->flip_arrow2 == flip_arrow2)	// error reporting
    pass ("Working Properly");
  else
    fail ("error in reading arrow1");
  ...
  act_measure = dwg_ent_dim_get_act_measurement (dim,
							  &error);
  if (!error  && dim->act_measurement == act_measure)	// error check
    pass ("Working Properly");
  else
