...
api_process (dwg_object *obj)
{
  ...
  if (!error && ins_pt.x == minsert->ins_pt.x && ins_pt.y == minsert->ins_pt.y
      && ins_pt.z == minsert->ins_pt.z)
    pass ("Working Properly");
  else
    fail ("error in reading insertion point");

  dwg_ent_minsert_get_scale (minsert, &scale, &error);
  if (!error && scale.x == minsert->scale.x && scale.y == minsert->scale.y
      && scale.z == minsert->scale.z)
    pass ("Working Properly");
  else
  ...
  if (!error && ext.x == minsert->extrusion.x && ext.y == minsert->extrusion.y
      && ext.z == minsert->extrusion.z)
    pass ("Working Properly");
  else
