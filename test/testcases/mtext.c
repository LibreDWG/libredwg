...
api_process (dwg_object *obj)
{
  ...
  if (!error && ext.x == mtext->extrusion.x && ext.y == mtext->extrusion.y
      && ext.z == mtext->extrusion.z) // error checking
    pass ("Working Properly");
  else
    fail ("error in reading extrusion");

  dwg_ent_mtext_get_insertion_pt (mtext, &ins_pt, &error);
  if (!error && ins_pt.x == mtext->insertion_pt.x
      && ins_pt.y == mtext->insertion_pt.y
      && ins_pt.z == mtext->insertion_pt.z) // error checking
    pass ("Working Properly");
  else
  ...
  if (!error && x_axis_dir.x == mtext->x_axis_dir.x
      && x_axis_dir.y == mtext->x_axis_dir.y
      && x_axis_dir.z == mtext->x_axis_dir.z) // error checking
    pass ("Working Properly");
  else
