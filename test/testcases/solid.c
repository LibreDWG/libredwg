...
api_process (dwg_object *obj)
{
  ...
  if (!error && ext.x == solid->extrusion.x && ext.y == solid->extrusion.y
      && ext.z == solid->extrusion.z) // error checks
    pass ("Working Properly");
  else
    fail ("error in reading extrusion");

  dwg_ent_solid_get_corner1 (solid, &c1, &error);
  if (!error && solid->corner1.x == c1.x
      && solid->corner1.y == c1.y) // error check
    pass ("Working Properly");
  else
    fail ("error in reading corner 1");

  dwg_ent_solid_get_corner2 (solid, &c2, &error);
  if (!error && solid->corner2.x == c2.x
      && solid->corner2.y == c2.y) // error check
    pass ("Working Properly");
  else
    fail ("error in reading corner 2");

  dwg_ent_solid_get_corner3 (solid, &c3, &error);
  if (!error && solid->corner3.x == c3.x
      && solid->corner3.y == c3.y) // error check
    pass ("Working Properly");
  else
    fail ("error in reading corner 3");

  dwg_ent_solid_get_corner4 (solid, &c4, &error);
  if (!error && solid->corner4.x == c4.x
      && solid->corner4.y == c4.y) // error check
    pass ("Working Properly");
  else
