...
api_process (dwg_object *obj)
{
  ...
  if (!error && point->x == points.x && point->y == points.y
      && point->z == points.z) // error check
    pass ("Working Properly");
  else
    fail ("error in reading point");

  dwg_ent_point_get_extrusion (point, &ext, &error);
  if (!error && ext.x == point->extrusion.x && ext.y == point->extrusion.y
      && ext.z == point->extrusion.z) // error checking
    pass ("Working Properly");
  else
