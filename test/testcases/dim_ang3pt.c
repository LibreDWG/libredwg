...
{
  int error;
  double act_measure, horiz_dir, lspace_factor, text_rot, ins_rot,
  ...
  if (!error && dim_ang3pt->def_pt.x == pt10.x
      && dim_ang3pt->def_pt.y == pt10.y && dim_ang3pt->def_pt.z == pt10.z)
    pass ("Working Properly");
  else
    fail ("error in reading def_pt");

  dwg_ent_dim_ang3pt_get_13_pt (dim_ang3pt, &pt13, &error);
  if (!error && dim_ang3pt->_13_pt.x == pt13.x
      && dim_ang3pt->_13_pt.y == pt13.y && dim_ang3pt->_13_pt.z == pt13.z)
    pass ("Working Properly");
  else
    fail ("error in reading pt13");

  dwg_ent_dim_ang3pt_get_14_pt (dim_ang3pt, &pt14, &error);
  if (!error && dim_ang3pt->_14_pt.x == pt14.x
      && dim_ang3pt->_14_pt.y == pt14.y && dim_ang3pt->_14_pt.z == pt14.z)
    pass ("Working Properly");
  else
    fail ("error in reading pt14");

  dwg_ent_dim_ang3pt_get_first_arc_pt (dim_ang3pt, &pt15, &error);
  if (!error && dim_ang3pt->first_arc_pt.x == pt15.x
      && dim_ang3pt->first_arc_pt.y == pt15.y
      && dim_ang3pt->first_arc_pt.z == pt15.z)
    pass ("Working Properly");
  else
