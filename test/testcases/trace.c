...
low_level_process (dwg_object *obj)
{
  ...
  printf ("extrusion of trace : x = %f, y = %f, z = %f\n", trace->extrusion.x,
          trace->extrusion.y, trace->extrusion.z);
...
api_process (dwg_object *obj)
{
  ...
  if (!error && ext.x == trace->extrusion.x && ext.y == trace->extrusion.y
      && ext.z == trace->extrusion.z) // error check
    pass ("Working Properly");
  else
    fail ("error in reading extrusion");

  dwg_ent_trace_get_corner1 (trace, &c1, &error);
  if (!error && trace->corner1.x == c1.x
      && trace->corner1.y == c1.y) // error check
    pass ("Working Properly");
  else
    fail ("error in reading corner 1");

  dwg_ent_trace_get_corner2 (trace, &c2, &error);
  if (!error && trace->corner2.x == c2.x
      && trace->corner2.y == c2.y) // error check
    pass ("Working Properly");
  else
    fail ("error in reading corner 2");

  dwg_ent_trace_get_corner3 (trace, &c3, &error);
  if (!error && trace->corner3.x == c3.x
      && trace->corner3.y == c3.y) // error check
    pass ("Working Properly");
  else
    fail ("error in reading corner 3");

  dwg_ent_trace_get_corner4 (trace, &c4, &error);
  if (!error && trace->corner4.x == c4.x
      && trace->corner4.y == c4.y) // error check
    pass ("Working Properly");
  else
