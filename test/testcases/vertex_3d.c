...
low_level_process (dwg_object *obj)
{
  ...
  printf ("point of vertex_3d : x = %f, y = %f, z = %f\n", vertex_3d->point.x,
          vertex_3d->point.y, vertex_3d->point.z);
}
...
api_process (dwg_object *obj)
{
  ...
  if (!error && flags == vertex_3d->flag) // error checking
    pass ("Working Properly");
  else
    fail ("error in reading flag");

  dwg_ent_vertex_3d_get_point (vertex_3d, &point, &error);
  if (!error && point.x == vertex_3d->point.x && point.y == vertex_3d->point.y
      && point.z == vertex_3d->point.z) // error check
    pass ("Working Properly");
  else
