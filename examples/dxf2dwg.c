...
// read from 0 SECTION 2 HEADER until 0 ENDSEC
// all known variables. warn about unknown vars, not error.
// TODO probably abuse header_variables.spec FIELD_* and add the DXF codes there.
static int
...
static int
dwg_read_dxf(char *filename, Dwg_Data * dwg)
{
  FILE *fh;
  struct stat attrib;
  struct Dwg_Header *dat = &dwg->header;
  ...
  if (!fh)
    {
      LOG_ERROR("Failed to read the file: %s", filename);
      return -1;
    }
  ...
  if (dxf_header_read (fh, dwg))
    if (dwg->header.version < R_13)
      goto fail;

  SINCE(R_2000) {
    if (dxf_classes_read (fh, dwg))
      goto fail;
  }

  if (dxf_tables_read (fh, dwg))
    goto fail;

  if (dxf_blocks_read (fh, dwg))
    goto fail;

  if (dxf_entities_read (fh, dwg))
    goto fail;

  SINCE(R_13) {
    if (dxf_objects_read (fh, dwg))
      goto fail;
  }

  SINCE(R_2000) {
    if (dxf_preview_read (fh, dwg))
      goto fail;
  }
