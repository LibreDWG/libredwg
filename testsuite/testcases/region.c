...
api_process(dwg_object *obj)
{
  ...
  if( point_error == 0 && point.x == region->point.x && point.y == region->point.y && point.z == region->point.z )
    {
      pass("Working Properly");
    }
  else
  ...
  if( isoline_present_error == 0 && isoline_present == region->isoline_present)
    {
      pass("Working Properly");
    }
  else
