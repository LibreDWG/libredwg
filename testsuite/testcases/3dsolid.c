...
api_process(dwg_object *obj)
{
  ...
  if (acis_empty_error == 0 && acis_empty == _3dsolid->acis_empty) // error reporting
    ...
    {
      fail("error in reading acis empty. dwg_ent_3dsolid_get_acis_empty not working");
    }
  ...
  if (version_error == 0 && version == _3dsolid->version) // error reporting
    ...
    {
      fail("error in reading version. dwg_ent_3dsolid_get_version is not working");
    }
  ...
  if( acis_data_error == 0 && acis_data == _3dsolid->acis_data) // error reporting
    ...
    {
      fail("error in reading acis data. dwg_ent_3dsolid_get_acis_data not working");
    }
  ...
  if( wireframe_data_present_error == 0 && wireframe_data_present == _3dsolid->wireframe_data_present) // error reporting
    ...
    {
      fail("error in reading wireframe data present. dwg_ent_3dsolid_get_wireframe_data_present is not working");
    }
  ...
  if( point_present_error == 0 && point_present == _3dsolid->point_present ) // error reporting
    ...
    {
      fail("error in reading point present. dwg_ent_3dsolid_get_point is not working");
    }
  ...
  if( point_error == 0 && point.x == _3dsolid->point.x && point.y == _3dsolid->point.y && point.z == _3dsolid->point.z ) // error reporting
    ...
    {
      fail("error in reading point. dwg_ent_3dsolid_get_point is not working properly");
    } 
  ...
  if( isoline_present_error == 0  && _3dsolid->isoline_present == isoline_present)
    {
      pass("Working Properly");
    }
  else
