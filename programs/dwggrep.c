...
do_match (char *filename, char *entity, int dxf, char* text, int textlen)
{
  ...
  else if (options & PCRE2_CASELESS)
    {
  ...
  for (j=i+1; j<argc; j++)
    {
      ...
      for (k=0; k < dwg.block_control.num_entries; k++)
        {
          found += match_BLOCK_HEADER(filename, dwg.block_control.block_headers[k]);
        }
      found += match_BLOCK_HEADER(filename, dwg.block_control.model_space);
      found += match_BLOCK_HEADER(filename, dwg.block_control.paper_space);
