...
#  ifndef DISABLE_DXF
#    ifndef DISABLE_JSON
  printf ("  -O fmt,  --format fmt     fmt: DWG, DXFB, JSON, GeoJSON\n");
#    else
  printf ("  -O fmt,  --format fmt     fmt: DWG, DXFB\n");
#    endif
#else
  printf ("  -O fmt,  --format fmt     fmt: DWG\n");
#  endif
...
#  ifndef DISABLE_DXF
#    ifndef DISABLE_JSON
  printf ("  -O fmt      fmt: DWG, DXFB, JSON, GeoJSON\n");
#    else
  printf ("  -O fmt      fmt: DWG, DXFB\n");
#    endif
#else
  printf ("  -O fmt      fmt: DWG\n");
#  endif
  printf (
...
#  endif
        ...
        case 'o':
          ...
          if (!fmt && outfile != NULL)
            {
              if (strstr (outfile, ".dwg") || strstr (outfile, ".DWG"))
                fmt = strdup ("dwg");
              else
#ifndef DISABLE_DXF
...
#endif
      if (!strcasecmp (fmt, "dwg"))
        {
          if (opts > 1 && outfile)
            fprintf (stderr, "Writing DWG file %s\n", outfile);
          error = dwg_encode (&dwg, &dat);
        }
#ifndef DISABLE_JSON
