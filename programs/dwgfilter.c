...
/*
 * dwgfilter.c: apply filters to all elements/objects/tables/header vars,
 *   see e.g. osmfilter.
 *   The filter can be either destructive, writing to a changed DWG (delete,
 *   transform, rename, ...)
 *   or purely informative, logging results from a query.
 *   The query expression looks a bit like
...
static int usage(void) {
  printf("\nUsage: dwgfilter [-Rr] expression *.dwg\n");
  return 1;
}
...
static int help(void) {
  printf("\nUsage: dwgfilter [OPTIONS]... expression files\n");
  printf("Apply filters to a list of DWGs. (WIP)\n\n");
...
// WIP parse and apply query
// check if pure (only search) or destructive (change dwgs in-place)
