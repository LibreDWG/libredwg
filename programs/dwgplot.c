...
/*
 * dwgplot.c: convert a DWG to GNU Plotutils formats.
 *    either without libplot to the basic ASCII plot format for | plot,
 *    or via libplot to its various supported formats directly.
...
const char *progname = "dwgplot";
const char *written = "Written by Reini Urban.";
...
static void output_plot (Dwg_Data *dwg);

static int
usage (void)
{
  printf ("\nUsage: dwgplot [-T format] [OPTIONS] DWGFILE | plot\n");
  return 1;
}
static int
opt_version (void)
...
help (void)
{
  printf ("\nUsage: dwgplot [OPTIONS]... DWGFILE >PLOTFILE\n");
  printf ("Converts some 2D elements of the DWG to a format supported by GNU Plotutils.\n"
          "\n");
#ifdef HAVE_GETOPT_LONG
# ifdef HAVE_PLOT_H
  printf ("  -T format, --output-format\n");
  // depends on libplot
  printf ("     One of: png, pnm, or gif (bitmap formats), or\n");
  printf ("             svg, ps, ai, cgm, fig, pcl, hpgl, regis, or tek (vector formats)");
  printf ("     The default format is meta i.e. plot(5) binary, which is probably not what you want.");
# endif
  printf ("  -o file, --file           optional output file, else stdout\n");
...
# ifdef HAVE_PLOT_H  
  ...
  printf ("     The default format is meta i.e. plot(5) binary, which is probably not what you want.");
# endif
  printf ("  -o file     optional output file, else stdout\n");
...
main (int argc, char *argv[])
{
  ...
  static struct option long_options[]
      = { { "verbose",       ARG_REQUIRED, &opts, 1 }, // optional
          { "output-format", ARG_REQUIRED, 0, 'T' },
          { "file",          ARG_REQUIRED, 0, 'o' },
          { "mspace",        ARG_NONE,     0, 'm' },
          { "force-free",    ARG_NONE,     0,  0 },
          { "help",          ARG_NONE,     0, 'h' },
  ...
  while
#ifdef HAVE_GETOPT_LONG
      ((c = getopt_long (argc, argv, "T:v:m::o:h", long_options, &option_index))
       != -1)
#else
      ((c = getopt (argc, argv, "T:v:m::o:hi")) != -1)
#endif
    {
      if (c == -1)
        break;
...
#  endif
  ...
  g_dwg.opts = opts;
  infile = argv[i];
  error = dwg_read_file (infile, &g_dwg);
...
#ifdef HAVE_PLOT_H  
  plotter = pl_newpl_r (output_format, NULL, outfh, stderr, plotter_params);
#endif
