...
my @defines = (
    '__GNUC__=4', '__x86_64__', '__inline=inline', '__THROW=',
    '__attribute__(x)='
);
if ( $^O =~ /darwin|bsd/ ) {
    push @defines,
        ( '__signed=signed', '__builtin_va_list=void*', '__extension__=' );
}
...
my $c = Convert::Binary::C->new->Include( '.', @ccincdir, @sys_includes )
    ->Define(@defines);
...
while (<$in>) {
    if ( !$n ) {
        ...
        if ( $n eq 'COMMON_TABLE_CONTROL_FIELDS' && $v eq 'entries' ) {
            $h{$n}{$_} = 'H*'
                for
                qw(block_headers layers styles linetypes views ucs vports apps
...
$SUBCLASSES{TABLECONTENTs}
    = [qw( AcDbLinkedTableData AcDbFormattedTableData AcDbTableContent)];
...
sub expand_define {
    ...
    if ( exists $defined->{$def} ) {
        ...
        if ( !$param )
        {    # replace the macro arg with $param? no. for now ignore
            for ( keys %{ $DXF{$def} } ) {
                $DXF{$n}->{$_} = $DXF{$def}->{$_};
            }
            for ( keys %{ $ENT{$def} } ) {
...
sub dxf_in {
    ...
    while (<$in>) {
        ...
        if ( !$n ) {
            ...
            my $dxf = $3;
            $type =~ s/0$//
                ;    # strip ending 0, optional dxf (default 0 not printed)
            $type =~ s/^BD1$/BD/;
            ...
            && /^\s+SUB_FIELD_HANDLE\s*\($v,\s*(\w+),\s*\d+,\s*(\d+)\)/ )
        {
            my $type = $1;
            $f = $1;
            $DXF{$n}->{$f} = $2 if $2;
        ...
        elsif ( @old && /^\s+SUB_FIELD_(.+?)\s*\(\w+,\s*(\w+),.*,\s*(\d+)\)/ )
        {
            ...
            my $sc = $1;
            if ( $sc eq 'AcDbLayout' )
            {    # FIXME: pop PLOTSETTINGS earlier in LAYOUT
...
$DXF{$_}->{'flag'}                     = 70
    for qw(VERTEX_3D VERTEX_MESH VERTEX_PFACE_FACE POLYLINE_PFACE);
...
my @annotscale
    = qw (TEXTOBJECTCONTEXTDATA MTEXTOBJECTCONTEXTDATA ALDIMOBJECTCONTEXTDATA
...
sub out_declarator {
    ...
    warn "no dxf for $key: $name 0\n"
        unless $dxf
        or ( $name eq 'parent' )
        or ( $key eq 'header_variables' and $name eq lc($name) );

    printf $fh
        "  { \"%s\",\t\"%s\", %s,  OFF (%s, %s),\n    %d,%d,%d, %d },\n",
        $name, $type, $size, $tmpl, $sname, $is_indirect, $is_malloc,
        $is_string, $dxf;
...
for ( sort @object_names ) {
    ...
    if ( is_table($_) ) {
        unshift @$aref,
            ...
            'AcDb' . $table_dxfname{$_} . 'TableRecord'
            );
    }
...
for (<DATA>) {
    ...
    if (/^(.*)\@\@(\w+ \w+)\@\@(.*)/) {
        ...
        if ( $tmpl =~ /^enum (\w+)/ ) {
            ...
            #print $fh "\n*/";
            my $i    = 0;
            my @keys = map { s/^DWG_TYPE__3D/DWG_TYPE_3D/; $_ }
                keys %{ $s->{enumerators} };
            for ( sort @keys ) {
                ...
                if ( $tmpl eq 'enum DWG_OBJECT_TYPE' ) {
                    ...
                    if ( $k =~ /^(BODY|REGION)$/ ) {
                        ...
                        =~ /^ASSOCARRAY(?:MODIFY|PATH|POLAR|RECTANGULAR)PARAMETERS$/
                        )
                    {
                    ...
                    {    # TODO subent, not entity
                        $size
                            = "sizeof (struct _dwg_entity_PROXY_LWPOLYLINE)";
                    }
                    $DWG_TYPE{$k} = $vs;
                    printf $fh
                        "  { \"%s\", %s /*(%d)*/, %s, %s },\t/* %d */\n",
         ...
         # find class type for this subclass. DWG_TYPE_parent if unique, or -1
                ...
                for ( sort keys %SUBCLASSES ) {
                    my $cl = $_;
                    if (   exists $entity_names{$cl}
                        or exists $object_names{$cl} )
                    {
                        my $a = $SUBCLASSES{$cl};
            ...
            print $doc
                "\n\@node ENTITIES\n\@section ENTITIES\n\@cindex ENTITIES\n\n";
            print $doc
                "All graphical objects with its fields. \@xref{Common Entity fields}\n\n";
            for (@entity_names) {
                ...
                if (    $typedef
                    and $typedef->{type} ne "struct _dwg_entity_$_" )
                {
            ...
            print $doc
                "\n\@node OBJECTS\n\@section OBJECTS\n\@cindex OBJECTS\n\n";
            print $doc
                "All non-graphical objects with its fields. \@xref{Common Object fields}\n\n";
            for (@object_names) {
                print $doc
                    "\@strong{$_} \@anchor{$_}\n\@cindex object, $_\n\@vindex $_\n";
                print $doc
                    "\@anchor{UNDERLAYDEFINITION}\n\@vindex UNDERLAYDEFINITION\n"
                    if /^PDFDEFINITION/;
                print $doc
                    "\@anchor{ASSOCARRAYPARAMETERS}\n\@vindex ASSOCARRAYPARAMETERS\n"
                    if /^ASSOCARRAYMODIFYPARAMETERS/;
                print $doc "\n";
                my $typedef = $c->typedef("Dwg_Object_$_");
                if (    $typedef
                    and $typedef->{type} ne "struct _dwg_object_$_" )
                {
                    my $type = expand_typedef( $typedef->{type} )
                        ;    # unify to one struct
                    if ($type) {
            ...
            for (@subclasses) {
                my ($name) = $_ =~ /^_dwg_(.*)/;
                print $doc
                    "\@strong{Dwg_$name} \@anchor{Dwg_$name}\n\@vindex Dwg_$name\n\n";
                if ( $unions{$_} ) {
            ...
            if ( $1 eq 'header_variables' ) {
                ...
                print $doc
                    "\@strong{Common Object fields} \@anchor{Common Object fields}\n";
                print $doc "\@cindex Common Object fields\n\n";
            }
            elsif ( $1 eq 'object_entity' ) {
                print $doc
                    "\@strong{Common Entity fields} \@anchor{Common Entity fields}\n";
                print $doc "\@cindex Common Entity fields\n\n";
            }
            elsif ( $1 eq 'summaryinfo' ) {
                print $doc
                    "\@strong{SummaryInfo fields} \@anchor{SummaryInfo fields}\n";
...
for (<$in>) {
    ...
    if (m{/\* \@\@for test_HEADER\@@ \*/}) {
        ...
        for my $d ( @{ $s->{declarations} } ) {
            ...
            my $stype = $type;
            $type = 'BITCODE_' . $type
                unless ( $type =~ /^(struct|Dwg_)/ or $type =~ /^[a-z]/ );
            if ( !$is_ptr ) {
                print $fh <<"EOF";
  {
    $type $var;
...
EOF
                if ( $type
                    =~ /(int|long|short|char ||double|_B\b|_B[BSLD]\b|_R[CSLD])/
                    )
                {
                    print $fh "    $var++;\n";
                }
...
EOF
                    ...
                    )
                {
                    print $fh "    $var--;\n";
                    print $fh
                        "    dwg_dynapi_header_set_value (dwg, \"$sname\", &$var, 0);\n";
                }
                print $fh "\n  }\n";
            }
            else {
...
EOF
    ...
    if (m{/\* \@\@for test_OBJECT\@\@ \*/}) {
      ...
      #TABLE is stored as fixedtype UNKNOWN_ENT, so the dynapi test would fail
            ...
            my ( $Entity, $lentity )
                = $is_ent ? ( 'Entity', 'entity' ) : ( 'Object', 'object' );
            my $xname  = $name =~ /^3/ ? "_$name" : $name;
            my $lname  = lc $xname;
...
EOF
           ...
           # LAYER.flag is computed since r2000
                ...
                if ( $var =~ /\./ )
                { # embedded structs, like ovr.name. some have fields, some not
                    next if $var =~ /^ovr\./;
                    $svar =~ s/\./_/g;
                    $var = $svar;
                }
                ...
                next
                    if $key eq 'evalexpr.value.text1'
                    ;    # already handled by evalexpr memcmp
                my $stype = $type;
                $type =~ s/D_1$/D/;
                $type = 'BITCODE_' . $type
                    unless ( $type =~ /^(struct|Dwg_)/ or $type =~ /^[a-z]/ );
...
EOF
                    ...
                    print $fh "\n  }\n";
                        ...
                        $type =~ /\*$/
                    and $type !~ /(RC\*|struct _dwg_object_)/

                    # no countfield
                    and $var
                    !~ /^(ref|block_size|extra_acis_data|objid_object_handles)$/
                    ...
                    )
                {
                    ...
                    my $countfield
                        = exists $countfield{$var}
                        ? $countfield{$var}
                        : "num_$var";
                    $countfield = 'numdashes'
                        if $name eq 'LTYPE' and $var =~ /^(styles|dashes)$/;
                    my $count = 1;
...
EOF
                    if ( $stype =~ /^(TV|T|TU|RC\*|unsigned char\*|char\*)$/ )
                    {
                        ...
                        print $fh "        && $svar\n";
                        print $fh
                            "           ? strEQ ((char *)$svar, (char *)$lname->$key)\n";
                        print $fh "           : !$lname->$key)\n";
                    }
                    elsif ( 0 and $stype =~ /^TF/ and $size !~ /^sizeof/ ) {
                        print $fh
                            "        && !memcmp ($svar, $lname->$skey, $size))\n";
                    }
                    elsif ( $type !~ /\*\*/ ) {
                        print $fh
                            "        && !memcmp (&$svar, &$lname->$skey, $size))\n";
    ...
    for my $name (@$names) {
        ...
        if ( $STABILITY->{$name} ) {
            ...
            if ( $s =~ /\$lname/ ) {
                ...
                # skip typedefs of
                if ( $lname
                    =~ /^(xline|vertex_mesh|vertex_pface|region|body)$/ )
                {
                    next;
                }
...
while (<$in>) {
    if (m/^\s*\/\* Start auto-generated/) {
        ...
        my %STABLEVAR2
            = map { $_ => 1 } qw(PDFDEFINITION DGNDEFINITION DWFDEFINITION);
        $tmpl
            = "  typedef struct _dwg_abstractobject_UNDERLAYDEFINITION\t\tdwg_obj_\$lname;\n";
        ...
        delete %UNSTABLE1{
            qw(ASSOCARRAYMODIFYPARAMETERS ASSOCARRAYPATHPARAMETERS
                ASSOCARRAYPOLARPARAMETERS ASSOCARRAYRECTANGULARPARAMETERS)
        };
        out_classes( $out, \@object_names, \%UNSTABLE1, $tmpl );
        my %UNSTABLE2 = map { $_ => 1 }
            qw(ASSOCARRAYMODIFYPARAMETERS ASSOCARRAYPATHPARAMETERS
            ASSOCARRAYPOLARPARAMETERS ASSOCARRAYRECTANGULARPARAMETERS);
        $tmpl
            = "  typedef struct _dwg_abstractobject_ASSOCARRAYPARAMETERS\t\tdwg_obj_\$lname;\n";
...
if (0) {
    ...
    while (<$in>) {
        if (m/^\s*\/\* Start auto-generated content/) {
            ...
            print $out "// clang-format: off\n";
            $tmpl
                = "int dwg_free_\$name (Bit_Chain *restrict dat, Dwg_Object *restrict obj);\n"
...
while (<$in>) {
    if (m/^\/\* Start auto-generated/) {
        ...
        $tmpl
            = "EXPORT Dwg_Entity_\$name** dwg_getall_\$name (Dwg_Object_Ref* hdr);\n";
        out_classes( $out, \@entity_names, \%STABLE, $tmpl );
        print $out "/* unstable */\n";
        out_classes( $out, \@entity_names, \%UNSTABLE, $tmpl );
        print $out "#ifdef DEBUG_CLASSES\n";
        ...
        $tmpl
            = "EXPORT Dwg_Object_\$name** dwg_getall_\$name (Dwg_Data* dwg);\n";
        out_classes( $out, \@object_names, \%STABLE, $tmpl );
        print $out "/* unstable */\n";
        out_classes( $out, \@object_names, \%UNSTABLE, $tmpl );
        print $out "#ifdef DEBUG_CLASSES\n";
        ...
        print $out "\n/* dwg_object_to_ API */\n";
        $tmpl
            = "EXPORT Dwg_Entity_\$name* dwg_object_to_\$name (Dwg_Object* obj);\n";
        out_classes( $out, \@entity_names, \%STABLE, $tmpl );
        print $out "/* unstable */\n";
        out_classes( $out, \@entity_names, \%UNSTABLE, $tmpl );
        print $out "#ifdef DEBUG_CLASSES\n";
        ...
        $tmpl
            = "EXPORT Dwg_Object_\$name* dwg_object_to_\$name (Dwg_Object* obj);\n";
        out_classes( $out, \@object_names, \%STABLE, $tmpl );
        print $out "/* unstable */\n";
        out_classes( $out, \@object_names, \%UNSTABLE, $tmpl );
        print $out "#ifdef DEBUG_CLASSES\n";
