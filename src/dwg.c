...
dat_mmap_file (Bit_Chain *restrict dat, int prot)
{
  ...
  if (fd < 0)
    {
      LOG_ERROR ("fileno failed with %s", strerror (errno));
      fclose (dat->fh);
      dat->fh = NULL;
      return DWG_ERR_IOERROR;
    }
  ...
  if ((prot & PROT_WRITE) && !dat->size)
    dat->size = 4096;
  if ((prot & PROT_WRITE) && ftruncate (fd, dat->size))
    {
  ...
  chain = mmap (NULL, dat->size, prot, flags, fd, 0L);
  if (!dat->size || !chain || chain == MAP_FAILED)
#else
  (void)prot;
#endif
    {
...
#ifdef HAVE_SYS_MMAN_H
  dat->chain = (unsigned char *)chain;
  dat->opts |= DWG_OPTS_MMAP;
#endif
  return 0;
}
...
dat_close_file (Bit_Chain *restrict dat)
{
  ...
  if (!dat->fh) // already closed
    return 0;
#ifdef HAVE_SYS_MMAN_H
  // msync only needed for mmaped buffers.
  if ((dat->opts & DWG_OPTS_MMAP) && dat->chain && dat->size
      && msync (dat->chain, dat->size, MS_SYNC))
    {
...
#ifdef HAVE_SYS_MMAN_H
  if ((dat->opts & DWG_OPTS_MMAP) && dat->chain)
    {
...
#    endif
  ...
  dat.fh = fopen (filename, "w+b");
  if (!dat.fh)
    {
      LOG_ERROR ("Failed to create the file: %s", filename);
      return error | DWG_ERR_IOERROR;
    }
  error = dat_mmap_file (&dat, PROT_READ | PROT_WRITE);
  if (error)
    return error;
#  endif
