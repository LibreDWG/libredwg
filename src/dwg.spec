...
DWG_ENTITY (PROXY_ENTITY)
  ...
  PRE (R_2018) {
    FIELD_BL (dwg_versions, 0);
    DECODER {
      FIELD_VALUE (maint_version) = FIELD_VALUE(dwg_versions) & 0xff;
      FIELD_VALUE (dwg_version) = FIELD_VALUE(dwg_versions) >> 16;
  }
  LATER_VERSIONS {
    FIELD_BL (dwg_version, 0);
    FIELD_BL (maint_version, 0);
    DECODER {
      FIELD_VALUE (dwg_versions) = (FIELD_VALUE(dwg_version) << 16) + FIELD_VALUE(maint_version);
    }
  }
  DXF {
    FIELD_BL (dwg_versions, 95);
  }
  SINCE (R_2000b) {
  ...
  DECODER {
    ...
    LOG_TRACE_TF (_obj->data, _obj->data_numbits);
  }
  ENCODER {
    LOG_TRACE ("data_numbits: " FORMAT_BL " [ 93]\n", _obj->data_numbits);
