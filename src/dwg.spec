...
{
  FIELD_RC (mtext_type,
            70); // 1=single line, 2=multi line attrib, 4=multi line attdef
...
VERSIONS (R_13b1, R_14)
{
  ...
  DXF
  {
    if (_obj->alignment_pt.x != 0.0 && _obj->alignment_pt.y != 0.0)
      {
        FIELD_2RD (alignment_pt, 11);
        FIELD_RD (elevation, 31);
      }
...
DWG_ENT_SUBCLASS (ATTRIB,
                  AcDbMTextObjectEmbedded); // lgtm[cpp/use-after-free]
                                            // codeql[cpp/use-after-free]
DWG_ENT_SUBCLASS (ATTDEF,
                  AcDbMTextObjectEmbedded); // lgtm[cpp/use-after-free]
                                            // codeql[cpp/use-after-free]
...
VERSIONS (R_2_0b, R_11)
{
  ...
  Dwg_Object *hdr = _ent->ownerhandle && _ent->ownerhandle->obj
                        ? _ent->ownerhandle->obj
                        : NULL;
  if (!hdr)
...
SINCE (R_13b1)
{
  ...
  if (_hdr
      && (bit_empty_T (dat, _obj->name) || bit_eq_T (dat, _obj->name, "*")))
    {
...
SINCE (R_13b1)
{
  ...
  ENCODER
  {
    if (FIELD_VALUE (start_width)
        && FIELD_VALUE (start_width) == FIELD_VALUE (end_width))
      {
...
#ifdef IS_DXF
{
  ...
  UNTIL (R_9)
  { VALUE_2RD (pt, 10) } LATER_VERSIONS
  { VALUE_3BD (pt, 10) } VALUE_RS ((BITCODE_RS)128, 70);
...
PRE (R_13b1)
{
  ...
  if (R11OPTS (OPTS_R11_POLYLINE_IN_EXTRA) && obj->size > 20)
    {
      // Note: layer is then the extras_start offset
      DECODER
      {
        _obj->extra_r11_size
            = (obj->address + obj->size - dat->byte) & 0xFFFFFFFF;
...
IF_FREE_OR_SINCE (R_2004a) { HANDLE_VECTOR (vertex, num_owned, 3, 0); }

IF_FREE_OR_SINCE (R_13b1) { FIELD_HANDLE (seqend, 3, 0); }
...
SUBCLASS (AcDbLine);
PRE (R_10)
{ FIELD_2RD (start, 10) FIELD_2RD (end, 11) } VERSIONS (R_10, R_12)
{
  if (!R11FLAG (FLAG_R11_HAS_ELEVATION))
  ...
  else
    {
      FIELD_2RD (start, 10)
      FIELD_2RD (end, 11)
      if (((!R11OPTS (1) && obj->size >= 56)
           || (R11OPTS (1) && obj->size >= 80))
...
SINCE (R_2000b)
{
  ENCODER
  {
    FIELD_VALUE (z_is_zero)
        = (FIELD_VALUE (start.z) == 0.0 && FIELD_VALUE (end.z) == 0.0);
  }
  DXF_OR_PRINT
  {
...
DECODER
{
  ...
  {
    BITCODE_RC flag = FIELD_VALUE (flag);
    flag = (FIELD_VALUE (flag2) & 1) ? flag | 0x80
                                     : flag & 0xBF; /* set bit 6 */
    FIELD_VALUE (flag) = flag;
  }
}
...
JSON{ FIELD_3RD (def_pt, 0) } PRE (R_13b1)
{
  if (R11OPTS (8))
    {
      if (dat->version >= R_10)
...
IF_FREE_OR_SINCE (R_2004a) { HANDLE_VECTOR (vertex, num_owned, 4, 0); }
SINCE (R_13b1)
{
  FIELD_HANDLE (seqend, 3, 0);
}
...
SINCE (R_2000b)
{
  ...
  else
  {
    ...
    FIELD_BD (FRONTZ, 43);
    FIELD_BD (BACKZ, 44);
    if (dwg->header.dwg_version != 0x1a)
      { // AC1020/R_2006 only here
        FIELD_BD (SNAPANG, 50);
        FIELD_2RD (VIEWCTR, 12);
        FIELD_2RD (SNAPBASE, 13);
      }
...
DXF { FIELD_HANDLE (visualstyle, 5, 348); }
...
/* removed: WIRESTRUCT_fields, 3DSOLID helpers, ACTION_3DSOLID - now in
 * dwg_spec_shared.h */
#if 0
...
#  else
          const uint8_t wstr1[]
              = { 'A', 0, 'C', 0, 'A', 0, 'D', 0, '_', 0, 'S', 0, 'O', 0,
                  'R', 0, 'T', 0, 'E', 0, 'N', 0, 'T', 0, 'S', 0, 0,   0 };
          const uint8_t wstr2[]
...
#  endif
          if (text
              && (bit_eq_TU (text, (BITCODE_TU)wstr1)
      ...
      else if (dxf == 350)
        {
          ...
          if (!text)
            ;
                   ...
                   || strEQc (text, "ACAD_FILTER") || strEQc (text, "SPATIAL"))
            dxf = 360;
        }
...
/* (46)
   See
   https://docs.intellicad.org/files/oda/2021_11/oda_drawings_docs/db_fcf.html
 */
...
FIELD_VECTOR (verts[rcount1].lines[rcount2].segparms, BD,
              verts[rcount1].lines[rcount2].num_segparms, 41)
...
FIELD_VECTOR (verts[rcount1].lines[rcount2].areafillparms, BD,
              verts[rcount1].lines[rcount2].num_areafillparms, 42)
...
DXF
{
  ...
  if (FIELD_VALUE (preview_size) && _obj->preview)
    {
  ...
  // The DXF TABLE.BLOCK_RECORD only has this. More later in the BLOCKS
  // section.
  return 0;
}
...
SINCE (R_13b1)
{
  FIELD_VALUE (flag) |= FIELD_VALUE (anonymous) | FIELD_VALUE (hasattrs) << 1
...
IF_FREE_OR_SINCE (R_13b1) { FIELD_HANDLE (endblk_entity, 3, 0); }
IF_FREE_OR_SINCE (R_2000b)
{
...
SINCE (R_2000b)
{
  ...
  ENCODER
  {
    ...
    if (!(dwg->opts & DWG_OPTS_INJSON))
      FIELD_VALUE (flag0)
          = (FIELD_VALUE (frozen) ? 1 : 0) + (FIELD_VALUE (off) ? 2 : 0)
  ...
  //      locked (4), is_xref_ref (16), is_xref_resolved (32), is_xref_dep
  //      (64).
  FIELD_VALUE (flag)
      = FIELD_VALUE (frozen) | (FIELD_VALUE (frozen_in_new) << 1)
        | (FIELD_VALUE (locked) << 2) | (FIELD_VALUE (is_xref_ref) << 4)
        | ((FIELD_VALUE (is_xref_resolved) ? 1 : 0) << 5)
        | (FIELD_VALUE (is_xref_dep) << 6);
  DECODER { LOG_TRACE ("  => flag %u [BL 70]\n", FIELD_VALUE (flag)); }
  JSON { FIELD_RC (linewt, 370); }
}
SINCE (R_13b1)
{
  DXF
...
VERSIONS (R_13b1, R_14)
{
  ...
  FIELD_VALUE (flag0)
      |= FIELD_VALUE (frozen) | (FIELD_VALUE (frozen_in_new) << 1)
         | (FIELD_VALUE (locked) << 2) | (FIELD_VALUE (off) ? 32 : 0);
  // for DXF
  FIELD_VALUE (flag)
      |= FIELD_VALUE (frozen) | (FIELD_VALUE (frozen_in_new) << 1)
         | (FIELD_VALUE (locked) << 2) | (FIELD_VALUE (off) ? 32 : 0);
    ...
    if (FIELD_VALUE (font_file) && !dxf_has_STYLE_eed (dat, obj->tio.object))
      {
        ...
        _buf[255] = '\0';
        if ((s = strstr (_buf, ".ttf")) || (s = strstr (_buf, ".TTF")))
          {
            *s = 0;
            VALUE_TFF ("ACAD", 1001);
...
SINCE (R_13b1)
{
  ...
  UNTIL (R_2004)
  {
    ...
    DECODER
    {
      ...
      for (rcount1 = 0; _obj->strings_area && rcount1 < _obj->numdashes;
           rcount1++)
        {
          if (_obj->dashes[rcount1].shape_flag & 2)
            {
              ...
              LOG_TRACE ("dashes[%u].text: \"%s\" [TF %" PRIuSIZE " 9] @%u\n",
                         rcount1, _obj->dashes[rcount1].text,
                         strlen (_obj->dashes[rcount1].text), dash_i);
              ...
              if (_obj->dashes[rcount1].shape_flag & 2)
                {
                  ...
                  LOG_TRACE ("dashes[%u].text: \"%s\" [TF %" PRIuSIZE
                             " 9] @%u\n",
                             rcount1, _obj->dashes[rcount1].text,
                             strlen (_obj->dashes[rcount1].text), dash_i);
                  dash_i += ((2
                              * bit_wcs2nlen (
                                  (BITCODE_TU)_obj->dashes[rcount1].text,
                                  256 - (dash_i / 2)))
                             + 2)
                            & 0xFFFF;
                }
...
PRE (R_13b1)
{
  ...
  SINCE (R_10)
  {
    ...
    DXF { FIELD_CAST (VIEWMODE, RS, 4BITS, 71); }
  }
}
LATER_VERSIONS
{
...
DXF
{ // has a different order of fields
  ...
  SINCE (R_2000b)
  {
    ...
    FIELD_RS (UCSVP,
              65); // in DWG as bit 0 of VIEWMODE. ODA bug, documented as 71
    FIELD_3BD (ucsorg, 110);
    FIELD_3BD (ucsxdir, 111);
    FIELD_3BD (ucsydir, 112);
    FIELD_HANDLE0 (named_ucs, 5, 345);
...
else
{
  ...
  PRE (R_13b1)
  {
    ...
    DECODER
    {
      FIELD_VALUE (view_width)
          = FIELD_VALUE (aspect_ratio) * FIELD_VALUE (VIEWSIZE);
  ...
  else // PRE (R_13
  {
    ...
    FIELD_BD (FRONTZ, 43);
    FIELD_BD (BACKZ, 44);
    // UCSFOLLOW is bit 3 of 71, UCSVP bit 0, ucs_at_origin bit 1. below
    // decoded again.
    FIELD_4BITS (VIEWMODE, 71);
    SINCE (R_2000b)
    {
    ...
    FIELD_B (FASTZOOM, 73);
    FIELD_BB (UCSICON,
              74); // bits 0 and 1 of 71: uscicon_on, ucsicon_at_origin
    FIELD_B (GRIDMODE, 76);
    FIELD_2RD (GRIDUNIT, 15);
    FIELD_B (SNAPMODE, 75);
...
DXF { FIELD_VALUE (flag) = FIELD_VALUE (flag0); }
...
IF_FREE_OR_SINCE (R_2010b)
{
  FIELD_B (DIMTXTDIRECTION,
           294);           // ODA documented as 295. Undocumented in ACAD
...
#ifdef IS_DXF
{
  ...
  if (end_angle < 0.0)
    end_angle += 360.0;
  while (start_angle > 91.0)
    start_angle -= 90.0;
  while (end_angle > 91.0)
    end_angle -= 90.0;
...
PRE (R_2018)
{
  ...
  switch (FIELD_VALUE (lines[rcount1].lt.index))
    ...
    case 32767:
      VALUE_TFF ("BYLAYER", 6);
      break; /* default (SHRT_MAX) */
    case 32766:
      VALUE_TFF ("BYBLOCK", 6);
      break;
    case 0:
      VALUE_TFF ("CONTINUOUS", 6);
      break;
...
#ifndef IS_INDXF
int DWG_FUNC_N (ACTION, _HATCH_gradientfill) (Bit_Chain *restrict dat,
                                              Bit_Chain *restrict str_dat,
                                              const Dwg_Object *restrict obj,
                                              Dwg_Entity_HATCH *restrict _obj);

int
DWG_FUNC_N (ACTION, _HATCH_gradientfill) (Bit_Chain *restrict dat,
                                          Bit_Chain *restrict str_dat,
                                          const Dwg_Object *restrict obj,
                                          Dwg_Entity_HATCH *restrict _obj)
{
    ...
    switch (FIELD_VALUE (segx[rcount2].curve_type))
      ...
      case 4: /* SPLINE */
        ...
        if (FIELD_VALUE (segx[rcount2].num_knots > 10000))
          {
            LOG_ERROR ("Invalid HATCH.paths[" FORMAT_BL "].segs[" FORMAT_BL
                       "].num_knots " FORMAT_BL,
                       rcount1, rcount2,
        ...
        FIELD_VECTOR (segx[rcount2].knots, BD, segx[rcount2].num_knots, 40);
...
#define control_points segx[rcount2].control_points
        REPEAT3 (segx[rcount2].num_control_points, control_points,
...
#define pline_paths paths[rcount1].polyline_paths
    REPEAT2 (paths[rcount1].num_segs_or_paths, pline_paths,
...
SUB_HANDLE_VECTOR (paths[rcount1], boundary_handles, num_boundary_handles, 4,
                   330);
...
ENCODER
{
  if (_obj->num_seeds && !_obj->seeds)
    {
      LOG_WARN ("HATCH.num_seeds %u but seeds NULL; force 0", _obj->num_seeds);
      _obj->num_seeds = 0;
    }
}
  ...
  if (_obj->num_seeds && avail_bits < (int64_t)_obj->num_seeds * 128)
    {
      ...
      LOG_WARN ("Truncate HATCH.num_seeds from %u to %u", _obj->num_seeds,
                max_seeds);
...
SINCE (R_2004a)
{
  if (_obj->is_gradient_fill)
    error
        |= DWG_FUNC_N (ACTION, _HATCH_gradientfill) (dat, str_dat, obj, _obj);
}
#endif
...
SINCE (R_2004a)
{
  error |= DWG_FUNC_N (ACTION, _HATCH_gradientfill) (dat, str_dat, obj,
...
// DECODER {
//   FIELD_VALUE (has_derived) =
//         FIELD_VALUE (has_derived) || (FIELD_VALUE (paths[rcount1].flag) &
//         0x4);
// }
VALUEOUTOFBOUNDS (paths[rcount1].num_boundary_handles, 10000)
SUB_HANDLE_VECTOR (paths[rcount1], boundary_handles, num_boundary_handles, 4,
...
{
  if (_obj->is_gradient_fill)
    error |= DWG_FUNC_N (ACTION, _HATCH_gradientfill) (
        dat, str_dat, obj, (Dwg_Entity_HATCH *)_obj);
}
#  endif
    ...
    if (_obj->plotsettings.plotview && !_obj->plotsettings.plotview_name)
      {
        _obj->plotsettings.plotview_name
            = dwg_handle_name (dwg, "VIEW", _obj->plotsettings.plotview);
        LOG_TRACE ("plotsettings.plotview_name => %s\n",
                   _obj->plotsettings.plotview_name);
      }
  }
    ...
    if (!bit_empty_T (dat, _obj->plotsettings.plotview_name))
      {
        _obj->plotsettings.plotview = dwg_find_tablehandle (
            dwg, _obj->plotsettings.plotview_name, "VIEW");
...
LATER_VERSIONS
{
  ...
  {
    if (!_obj->plotsettings.plotview
        && !bit_empty_T (dat, _obj->plotsettings.plotview_name))
      {
        _obj->plotsettings.plotview = dwg_find_tablehandle (
            dwg, _obj->plotsettings.plotview_name, "VIEW");
        if (_obj->plotsettings.plotview)
  ...
  FIELD_HANDLE (plotsettings.plotview, 4,
                0); // lgtm[cpp/use-after-free] codeql[cpp/use-after-free]
  DECODER
  {
...
FREE
{
  FIELD_TV (plotsettings.plotview_name, 6);
  FIELD_HANDLE (plotsettings.plotview, 5, 6);
}
...
SINCE (R_2007a)
{
  FIELD_HANDLE (plotsettings.shadeplot, 4,
                333); // lgtm[cpp/use-after-free] codeql[cpp/use-after-free]
}

SUBCLASS (AcDbLayout)
FIELD_T (layout_name, 1);
DXF { FIELD_BS (layout_flags, 70); }
...
VALUE_BS (((FIELD_VALUE (flag) & 256) ? 128 : 0)
              + (FIELD_VALUE (flag) & 512 ? 1 : 0),
          70);
FIELD_BD (const_width, 43);
#else
FIELD_BS (
    flag,
    70); // 512 closed, 256 plinegen, 4 constwidth, 8 elevation, 2 thickness
...
if (FIELD_VALUE (num_points) > 20000)
  {
    LOG_ERROR ("Invalid LWPOLYLINE.num_points %ld",
               (long)FIELD_VALUE (num_points));
    _obj->num_points = 0;
    DEBUG_HERE_OBJ
    return DWG_ERR_VALUEOUTOFBOUNDS;
  }
...
IF_FREE_OR_SINCE (R_2000b) { FIELD_2DD_VECTOR (points, num_points, 10); }
...
SINCE (R_2018)
{
  ...
  FIELD_VALUE (version)
      = (FIELD_VALUE (maint_version) << 8) + FIELD_VALUE (dwg_version);
  DXF { FIELD_BLx (version, 95); }
}
SINCE (R_2000b)
{
...
DECODER
{
  ...
  LOG_TRACE_TF (_obj->data,
                (_obj->data_numbits / 8) + (_obj->data_numbits % 8 ? 1 : 0));
}
ENCODER
{
  LOG_TRACE ("data_numbits: " FORMAT_BL " [ 93]\n", _obj->data_numbits);
...
FREE { FIELD_BINARY (data, _obj->data_numbits / 8, 0); }
    ...
    else
      {
        ...
        switch (code)
          ...
          case 5:
            ...
            break;
          default:
            LOG_ERROR ("Illegal %s objids[%u].code %u", obj->name, rcount1,
                       code);
          }
...
#ifdef IS_DXF
PRE (R_2000b)
{ SUBCLASS (AcDbZombieObject) } LATER_VERSIONS
...
while (hdl_dat->byte < hdl_dat->size - 1)
  {
    ...
    if (bit_read_H (hdl_dat, &hdl))
      ...
      {
        BITCODE_H ref = dwg_add_handleref (dwg, hdl.code, hdl.value, NULL);
...
DXF { HANDLE_VECTOR (childs, num_childs, 3, 360); }
FIELD_BL (num_objects, 97);
VALUEOUTOFBOUNDS (num_objects, 20000)
DXF { HANDLE_VECTOR (objects, num_objects, 5, 331); }
#ifndef IS_DXF
PRE (R_2007a)
{
  FIELD_TV (format, 4);
...
TABLE_value_fields (
    value) // lgtm[cpp/use-after-free] codeql[cpp/use-after-free]
    if (error & DWG_ERR_INVALIDTYPE) return error;
...
DECODER
{
  ...
  if (_obj->num_childval
      && (int64_t)_obj->num_childval * (int64_t)sizeof (Dwg_FIELD_ChildValue)
             > avail_bits)
    {
      LOG_WARN ("Truncate FIELD.num_childval from %u to 0",
                _obj->num_childval);
      _obj->num_childval = 0;
    }
}
...
TABLE_value_fields (childval[rcount1].value) if (error & DWG_ERR_INVALIDTYPE)
{
