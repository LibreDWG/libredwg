...
{
  static const unsigned char sentinels[DWG_SENTINEL_R11_AUX_HEADER_END + 2][16]
  ...
  if ((int)sentinel_id < 0 || (int)sentinel_id > DWG_SENTINEL_R11_AUX_HEADER_END)
    {
      fprintf (stderr, "ERROR: sentinel_id %d out of bounds\n", (int)sentinel_id);
      return sentinels[DWG_SENTINEL_R11_AUX_HEADER_END + 1];
    }
