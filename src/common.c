  ...
  static unsigned char gdst[9][16] =
    {
      ...
      { 0x8D, 0xA1, 0xC4, 0xB8, 0xC4, 0xA9, 0xF8, 0xC5, 0xC0, 0xDC, 0xF4, 0x5F,
          0xE7, 0xCF, 0xB6, 0x8A },
      { 0x72, 0x5L, 0x3B, 0x47, 0x3B, 0x56, 0x07, 0x3A, 0x3F, 0x23, 0x0B, 0xA0,
          0x18, 0x30, 0x49, 0x75 },
...
dwg_var_map(Dwg_Version_Type version, int index)
{
  ...
  for (i = 0; i < index; i++, res += 3)
    {
      while ((map[res] != R_BEFORE && version < map[res]) || (map[res + 1]
          != R_AFTER && version > map[res + 1]))
        {
          if (map[res] == DWG_END_OF_HEADER_VARIABLES)
            return DWG_END_OF_HEADER_VARIABLES;
          res += 3;
        }
    }
