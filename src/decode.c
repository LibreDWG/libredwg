      ...
      if (crc != crc2)
        {
          LOG_ERROR ("Object Section[%ld] CRC mismatch %04X <=> %04X",
                     (long)dwg->header.section[SECTION_OBJECTS_R13].number,
                     crc, crc2);
          // fails with r14
          // if (dwg->header.version == R_2000)
          //  return DWG_ERR_WRONGCRC;
              ...
              if (dwg_resolve_handleref (&ref, _obj))
                {
                  ...
                  for (j = 0; j < appid->num_entries; j++)
                    {
                      if (appid->entries[j]
                          && appid->entries[j]->absolute_ref == ref.absolute_ref)
                        {
  ...
  while (dat->byte < end_address)
    {
      ...
      switch (get_base_value_type (rbuf->type))
        ...
        case VT_INVALID:
        default:
          LOG_ERROR ("Invalid group code in xdata[%d]: %d", num_xdata, rbuf->type)
          dwg_free_xdata_resbuf (rbuf);
