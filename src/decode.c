  ...
  while (src->byte < src->size && dec->byte < dec->size && opcode1 != 0x11)
    {
      ...
      if ((long)pos >= (long)comp_offset &&
          (long)(pos + (comp_bytes - comp_offset)) < (long)dec->size)
        {
  ...
  for (i = j = 0; i < info->num_sections; ++i, ++j)
    {
      ...
      if (info->compressed == 2 && bytes_left > 0
          && (j * info->max_decomp_size) <= max_decomp_size)
        {
          ...
          dec.size = dec.byte + info->max_decomp_size;
...
#ifdef DEBUG
  // sanity checks
  //if (data->u.eed[idx].code == 0 || data->u.eed[idx].code == 4)
  //  assert (data->u.eed[idx].data->u.eed_0.length <= size - 1);
  //if (data->u.eed[idx].code == 10) // 3 double
  //  assert (size >= 1 + 3 * 8);
#endif
